...
void EnvironmentLight::init() {
  ...
  // Store the marginal distribution for y to marginal_y. Make sure to normalize pdf_envmap.
    ...
    for(int j = 0; j < h; j ++){
        marginal_y[j] = last_my;
        for(int i = 0; i < w; i ++){
            marginal_y[j] += pdf_envmap[w * j + i];
        }
        last_my = marginal_y[j];
    }
  ...
  // Store the conditional distribution for x given y to conds_y
    ...
    for (int j = 0; j < h; ++j) {
        double F_i = 0;
        for (int i = 0; i < w; ++i) {
            F_i += i > 0? pdf_envmap[w * j + i - 1] : 0;
            conds_y[w * j + i] = F_i;
            conds_y[w * j + i] /= j > 0? (marginal_y[j] - marginal_y[j - 1]) : marginal_y[j];
        }
    }
...
//
  ...
  // TODO: 3.3
    ...
    Vector2D ss = sampler_uniform2d.get_sample();
    uint32_t w = envMap->w, h = envMap->h;
    int y = std::upper_bound(marginal_y, marginal_y + h, ss.y) - marginal_y;
    int x = std::upper_bound(conds_y + y * w, conds_y + (y + 1) * w, ss.x) - (conds_y + y * w);
    Vector2D theta_phi = xy_to_theta_phi(Vector2D(x,y));
    *wi = theta_phi_to_dir(theta_phi);
    *distToLight = INF_D;
    *pdf = pdf_envmap[y * w + x] * (double)w * (double)h / 2. / pow(PI, 2)/ sin(theta_phi.x);
...
Spectrum EnvironmentLight::sample_dir(const Ray& r) const {
  // TODO: 3.1
    ...
    Vector2D vecxy = theta_phi_to_xy(theta_phi);
