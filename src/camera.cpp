...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
    ...
    Vector3D pLens(lensRadius * sqrt(rndR) * cos(2*PI*rndTheta), lensRadius*sqrt(rndR) * sin(2*PI*rndTheta), 0);
    
    double width = 2.0 * tan(radians(hFov)*.5);
    double height = 2.0 * tan(radians(vFov)*.5);
    
    double x_w = (x - 0.5) * width;
    double y_h = (y - 0.5) * height;
    
    Vector3D direction(x_w,y_h,-1);
    Vector3D pFocus = direction * focalDistance;
    Vector3D dir = pFocus - pLens;
    dir.normalize();
    dir = c2w * dir;
    pLens = c2w * pLens;
    
    Ray blue(pos + pLens, dir);
    blue.min_t = nClip;
    blue.max_t = fClip;
